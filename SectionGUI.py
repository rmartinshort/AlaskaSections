...
class SectionGUI(Frame):
	...
	def __init__(self,parent,width=1200,height=600,**options):
		...
		self.map = Basemap(ax=self.a,lat_0=62.5,lon_0=-147.0,resolution ='l',llcrnrlon=self.minlon,llcrnrlat=self.minlat,urcrnrlon=self.maxlon,urcrnrlat=self.maxlat)
		self.map.fillcontinents()
		self.map.drawcoastlines(linewidth=0.1)

		#Set up lines of lat/lon 
		p1 = np.arange(50,75,2.0)
		m1 = np.arange(-175,-120,4.0)
	...
	def SetElements(self):
		...
		Button(self, text='Start drawing',pady=0,padx=2).grid(row=13,columnspan=2,column=2,sticky=W+E)
		Button(self, text='Stop drawing',pady=0,padx=2).grid(row=13,columnspan=2,column=6,sticky=W+E)
